      ...
      <div className="space-y-3 p-5 text-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-[#1a1a1a]">{flightBooking.offer.title}</p>
            <p className="text-xs text-gray-500">{flightBooking.offer.location}</p>
          </div>
          <p className="font-semibold text-[#1a1a1a]">{formatUsd(flightBooking.totalPrice)}</p>
        </div>
        <div className="flex items-start gap-3 border-t border-slate-100 pt-3">
          <Building2 size={16} className="mt-0.5 shrink-0 text-[#2D83C2]" />
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-[#1a1a1a] line-clamp-2">{hotelBooking.offer.title}</p>
