...
function FlightSearchResults() {
  ...
  const depart = searchParams.get("depart") || defaultDepartDate();
  ...
  const canLiveSearch = Boolean(from.trim() && to.trim());
  ...
  useEffect(() => {
    ...
    fetch(`/api/flights/search?${params}`, { signal: controller.signal })
      .then(async (r) => {
        ...
        if (!r.ok && !data.error) throw new Error("flight search failed");
        return data;
      })
      .then((data) => {
        if (data.error) {
...
function SearchRouter() {
  ...
  if (type !== "flights") return <StaysSearchResults />;
